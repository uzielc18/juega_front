...
export function init_app(configService: AppService, injector: Injector): () => Observable<boolean> {
...
export class AppService {
    ...
    init(injector: Injector): Observable<boolean> {
        ...
        return auth.isAuthenticated().pipe(
            switchMap((status: boolean) => status ? this.httpClient.post(this._userInfoUrl, {
                id_padre: this.options.moduleId
            }).pipe(map((data: any) => data.data), map((data: any) => {
                if (data) {
                    if (data.hasOwnProperty('user') && Object.keys(data.user).length > 0) {
                        this._user = data.user;
                    }
                    if (data.hasOwnProperty('menu') && Array.isArray(data.menu)) {
